...
export default function LoginPage() {
  ...
  const checkRedirect = useCallback(
    (u: any) => {
      ...
      //
      // A guardian with no children used to be diverted to /complete-profile
      // instead. That fired on every login until they added one, and the
      // portal already handles the empty case better than the wizard did —
      // it asks for the child inline, on the page they were trying to reach.
      ...
      <div className="animate-fade-in-up w-full max-w-[440px] space-y-8">
        ...
        <div className="border-border bg-card rounded-3xl border p-8 shadow-[0_24px_60px_color-mix(in_oklch,var(--foreground)_4%,transparent),0_4px_12px_color-mix(in_oklch,var(--foreground)_2%,transparent)] md:p-10">
          ...
          <div className="mb-6 grid grid-cols-2 gap-3">
            {googleOAuthConfigured ? (
              <GoogleAuthButton
                roleHint={loginAs === "guardian" ? "GUARDIAN" : "USER"}
                disabled={loading}
                onAuthenticated={checkRedirect}
              />
            ) : (
              <GoogleAuthButtonUnavailable />
            )}
