...
export default function UsersPage() {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <Card className="border border-neutral-200/80 shadow-[0_4px_24px_rgba(0,0,0,0.015)] rounded-3xl p-6 bg-white space-y-6">
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-neutral-100">
                <th className="pb-3 text-[10px] font-bold uppercase tracking-wider text-neutral-400">User</th>
                <th className="pb-3 text-[10px] font-bold uppercase tracking-wider text-neutral-400">Role</th>
                <th className="pb-3 text-[10px] font-bold uppercase tracking-wider text-neutral-400">Status</th>
                <th className="pb-3 text-[10px] font-bold uppercase tracking-wider text-neutral-400">Joined Date</th>
                <th className="pb-3 text-[10px] font-bold uppercase tracking-wider text-neutral-400 text-right">Actions</th>
              </tr>
            </thead>
            <tbody>
              {usersLoading ? (
                [...Array(3)].map((_, i) => (
                  <tr key={i} className="border-b border-neutral-50 last:border-0">
                    ...
                    <td className="py-4"><div className="h-4 w-10 bg-neutral-100 animate-pulse rounded ml-auto" /></td>
                  </tr>
                ))
                ...
                filteredUsers.map((user) => {
                  ...
                  return (
                    <tr key={user.id} className="border-b border-neutral-50 last:border-0 hover:bg-neutral-50/50 transition-colors">
                      <td className="py-4">
                        <div className="flex items-center gap-3">
                          <div className="w-9 h-9 rounded-xl bg-neutral-900 text-white font-bold font-display text-xs flex items-center justify-center">
                            {getUserName(user) ? getUserName(user)[0].toUpperCase() : "U"}
                          </div>
                          <div>
                            <p className="text-xs font-semibold text-neutral-900">{getUserName(user)}</p>
                      ...
                      <td className="py-4 text-[10px] text-neutral-400 font-medium">
      ...
      <Dialog open={isUserDialogOpen} onOpenChange={setIsUserDialogOpen}>
        <DialogContent className="max-w-md rounded-2xl p-6 bg-white border border-neutral-200">
          ...
          <form onSubmit={handleSaveUser} className="space-y-4">
            ...
            <div className="space-y-1">
              <Label className="text-[10px] font-bold text-neutral-400 uppercase tracking-wider">Email Address (Read-only)</Label>
              <Input
                value={selectedUser?.email || ""}
                disabled
                className="h-10 text-xs border-neutral-200 bg-neutral-50 text-neutral-400"
              />
            </div>

            <div className="space-y-1">
              <Label className="text-[10px] font-bold text-neutral-400 uppercase tracking-wider">System Role</Label>
              <select
                value={userRoleId}
